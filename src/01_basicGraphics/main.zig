...
fn setupGpu(mode: gpuc.VideoMode, width: u32, height: u32) void {
...
pub fn main() noreturn {
    ...
    // in runtime/logging.zig and does basically the same things we did in the
    // previous example.
    //
    // Afterwards, we may use `std.log.info` to print messages to SIO1.
    ...
    psx.GPU_GP0.* = gpuc.gp0FbOffset1(0, 0);
    psx.GPU_GP0.* = gpuc.gp0FbOffset2(screen_width - 1, screen_height - 1);
    psx.GPU_GP0.* = gpuc.gp0FbOrigin(0, 0);
    ...
    waitForGp0Ready();
    psx.GPU_GP0.* = gpuc.gp0VramFill(.{ .r = 32, .g = 76, .b = 79 });
    psx.GPU_GP0.* = gpuc.gp0XY(0, 0);
    ...
    waitForGp0Ready();
    psx.GPU_GP0.* = gpuc.gp0ShadedTriangle(.{ .r = 255, .g = 0, .b = 0 }, true, false, false);
    psx.GPU_GP0.* = gpuc.gp0XY(screen_width / 2, 32);
    psx.GPU_GP0.* = gpuc.gp0Rgb(.{ .r = 0, .g = 255, .b = 0 });
    psx.GPU_GP0.* = gpuc.gp0XY(32, screen_height - 32);
    psx.GPU_GP0.* = gpuc.gp0Rgb(.{ .r = 0, .g = 0, .b = 255 });
    psx.GPU_GP0.* = gpuc.gp0XY(screen_width - 32, screen_height - 32);

    // Send two GP1 commands to set the origin of the area we want to display
    // and switch on the display output.
    waitForGp0Ready();
    psx.GPU_GP1.* = gpuc.gp1FbOffset(0, 0);
